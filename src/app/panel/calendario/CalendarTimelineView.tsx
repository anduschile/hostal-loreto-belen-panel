...
import { EXTERNAL_ROOM_ID } from "@/lib/data/calendar";
                    ...
                    <div className="flex flex-col">
                        {rooms.map((room) => {
                            ...
                            return (
                                <div
                                    key={room.id}
                                    className={`flex border-b ${isExternal ? 'min-h-[140px]' : 'min-h-[70px]'} relative group hover:bg-gray-50 transition-colors`}
                                >
                                    {/* 
                                    ...
                                    */}
                                    ...
                                    <div className="flex flex-1 relative">
                                        ...
                                        {days.map((day) => {
                                            if (isExternal) {
                                                ...
                                                return (
                                                    <div
                                                        key={day.toISOString()}
                                                        className="flex-1 min-w-[80px] border-r border-gray-100 h-full relative p-1"
                                                    >
                                                        <CalendarExternalCell
                                                            date={day}
                                                            reservations={dayReservations}
                                                            onReservationClick={onReservationClick}
                                                            viewMode={viewMode === 'day' ? 'day' : 'week'} // Default to week logic unless explicit day view
                                                        />
                                                    </div>
                                                );
                                            }
                                        ...
                                        {/* BLOQUES DE RESERVA (SOLO INTERNAL) */}
